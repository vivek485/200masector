...
def plot_stock_chart(df, symbol):
    ...
    # Add vertical dotted line at 9:15 candle for each day
    nine_fifteen_times = [idx for idx in df.index if idx.time() == pd.Timestamp('09:15:00').time()]
    ...
    async with sem:
        try:
            ...
            async with session.get(url, headers=headers) as response:
                ...
                # Add 200 MA
                final_df['ma200'] = ta.trend.sma_indicator(final_df['Close'], window=200)
                ...
                final_df['ema200'] = ta.trend.ema_indicator(final_df['Close'], window=200)
                ...
                # For previous day only
                prev_day = prev_date_df
                # Calculate buy/sell signals using np.where for cross above/below 200 EMA
                close = prev_day['Close']
                ema200 = prev_day['ema200']
                signalbuy = np.where((close.shift(1) < ema200.shift(1)) & (close > ema200), 1, 0)
                signalsell = np.where((close.shift(1) > ema200.shift(1)) & (close < ema200), 1, 0)
                cross_above = signalbuy.sum() >= 1
                cross_below = signalsell.sum() >= 1
                ...
                return final_df, cross_above, cross_below
...
if st.button("Get Signals"):
    ...
    with st.spinner("Scanning stocks..."):
        ...
        for stock_data, cross_above, cross_below in results:
            if stock_data is not None:
                ...
                if cross_above and prevclose > prhigh:
                    buy_stocks.append((stock_data, symbol))
                elif cross_below and prevclose < prlow:
